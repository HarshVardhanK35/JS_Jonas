 ...
 * 
 * - if we view inside network tab of the webpage: we get to see that the AJAX calls / requests are happening one after another but why next request(s) has to wait for completion of 1st or previous request(s) 
 ...
 * 
 * ? 1. Promise.race()
 * - this is also a Promise combinator instead of returning an array of results like "Promise.all()" this returns a single result that is faster than other promises
